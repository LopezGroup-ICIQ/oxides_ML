...
def get_voronoi_neighbourlist(atoms: Atoms, 
    ...
    if num_adsorbate_atoms == len(atoms):
        pairs = []
...
def atoms_to_nx(atoms: Atoms,
    ...
    # Case 1: Gas-phase system (all atoms are adsorbates)
    if len(adsorbate_idxs) == len(atoms):
        graph_nodes = list(range(len(atoms)))  # All atoms are part of the adsorbate
        graph = Graph()
        graph.add_nodes_from(graph_nodes)
        set_node_attributes(graph, {i: atoms[i].symbol for i in graph_nodes}, "elem")
        
        # Add edges based on Voronoi bonds
    ...
    # Case 2: Slab with Adsorbate
    # Get the Voronoi neighbour list for all atoms
    neighbour_list = get_voronoi_neighbourlist(atoms, voronoi_tolerance, scaling_factor, adsorbate_indices)

    # Get surface neighbors (atoms that are near the adsorbate but not part of it)
    ...
    # If surface_order > 0, include second-order surface neighbors
    if surface_order > 0:
    ...
    # Step 4: Create the graph by combining adsorbate atoms and relevant surface atoms
    if len(adsorbate_idxs) == 0:  # Bare slab case
        #graph_nodes = list(surface_neighbours_idxs)
        graph_nodes = list(set(i for pair in neighbour_list for i in pair))
    else:
        graph_nodes = list(adsorbate_idxs | surface_neighbours_idxs)  # Combine adsorbates and relevant surface atoms

    graph = Graph()
    graph.add_nodes_from(graph_nodes)
    set_node_attributes(graph, {i: atoms[i].symbol for i in graph_nodes}, "elem")

    # Step 5: Add edges based on Voronoi neighbour list
    ensemble_neighbour_list = [pair for pair in neighbour_list if pair[0] in graph.nodes and pair[1] in graph.nodes]
    graph.add_edges_from(ensemble_neighbour_list, ts_edge=0)
...
def atoms_to_pyg(atoms: Atoms,
    ...
    Args:
        ...
        surface_order (int): Order of neighboring surface atoms to include (use -1 for full slab).
        one_hot_encoder (OneHotEncoder): One-hot encoder for atomic elements.
        adsorbate_indices (list[int]): List of atom indices belonging to the adsorbate.

    Returns:
        graph (torch_geometric.data.Data): Graph representation of the system.
    """
    if calc_type not in ["int", "ts"]:
        raise ValueError("calc_type must be either 'int' or 'ts'.")

    nx, surface_neighbors, bb_idxs = atoms_to_nx(
        atoms, voronoi_tol, scaling_factor, surface_order, adsorbate_indices, calc_type
    )
    ...
    # edge_tails_heads = [(nodes_list.index(i), nodes_list.index(j)) for i, j in nx.edges]
    ...
    if calc_type == "ts":
        for i in range(edge_index.shape[1]):
            a, b = nodes_list[edge_index[0, i].item()], nodes_list[edge_index[1, i].item()]

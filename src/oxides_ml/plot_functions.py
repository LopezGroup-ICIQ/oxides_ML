...
from oxides_ml.functions import split_percentage
...
def pred_real(model,
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(figsize=(13, 4), ncols=3, dpi=500)
    true_lst_lst = []
...
def training_plot(train: list, val: list, test: list, split: int):
    ...
    for item in epochs:
        item += 1
    fig, ax = plt.subplots(figsize=(15/2.54, 10/2.54), dpi=500, layout="constrained")

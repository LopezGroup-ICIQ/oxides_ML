...
PATH_TO_ID_CACHE_FILE = "path_to_id_cache.json"

if os.path.exists(PATH_TO_ID_CACHE_FILE):
    with open(PATH_TO_ID_CACHE_FILE, "r") as f:
        path_to_id_cache = json.load(f)
else:
    path_to_id_cache = {}

def get_path_id(path: str, offline: bool = False) -> int:
    """
    Return unique ID for a given path.
    Cache the result in JSON file, creating new IDs incrementally.

    Args:
        path (str): Path string (unique per graph instance).
        offline (bool): If True, only use cached entries, else raise if missing.

    Returns:
        int: Unique ID assigned to path.
    """
    global path_to_id_cache

    if path in path_to_id_cache:
        return path_to_id_cache[path]

    if offline:
        raise RuntimeError(f"Path ID not found in cache and offline=True: {path}")

    # Assign new ID: max existing + 1 or 0 if empty
    new_id = max(path_to_id_cache.values(), default=-1) + 1
    path_to_id_cache[path] = new_id

    # Save updated cache
    with open(PATH_TO_ID_CACHE_FILE, "w") as f:
        json.dump(path_to_id_cache, f, indent=2)
...
class OxidesGraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # Preload path IDs to avoid calls in multiprocessing
        for path in vasp_files:
            try:
                get_path_id(path)  # populates the cache file gradually
            except Exception as e:
                print(f"Failed to preload path ID for {path}: {e}")
    ...
    def atoms_to_data(self, structure: Union[Atoms, str], 
        ...
        graph.graph_id = get_path_id(path, offline=True)
